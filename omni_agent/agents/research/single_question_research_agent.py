...
from omni_agent.core.tools import groq_search_tool, scrape_websites_tool
...
def enforce_tool_call_limits(
    ...
    print(f"Enforcing tool call limits for {tool.name}")
...
- **AS SOON AS YOU HAVE URLS USE THE SCRAPE_TOOL**
...
HARD CAP ON TOOL CALLS (strict):
- You may call 'search_tool' at most {MAX_NUMBER_OF_SEARCH_TOOL_CALLS} times total for this task.
- You may call 'scrape_tool' at most {MAX_NUMBER_OF_SCRAPE_TOOL_CALLS} times total for this task.
- If a cap is reached, DO NOT attempt additional tool calls; synthesize using existing evidence and explicitly note the limitation.
